...
raises on the full-embed prefix — don't waste time trying to just un-skip it.
...
## Selective type reindexing

`create-mapping` and `create-mapping-on-deploy` expose explicit
`--selective-reindex` mode. The comparison state is generated by
`snovault/elasticsearch/calculated_property_signature.py` and persisted in each
type mapping's `_meta`; it covers effective inherited/overridden calculated
properties and calculated properties from embedded types, walks factory helper
methods transitively (helpers-of-helpers, properties, pyramid `reify`), and
includes class-attribute configuration that changes indexed content without
changing the mapping: `rev` definitions, `filtered_rev_statuses`, `name_key`,
and the `__acl__`/`STATUS_ACL` inputs to the indexed `principals_allowed`.
Module-global loggers and builtin members (e.g. `self.__class__`) are treated
as inert so real types stay `complete`. A missing or incomplete
signature deliberately rebuilds and fully queues that type. Legacy `--check-first`
continues its mapping-only comparison, and `--wipe-es` remains a full reindex.
Selective queueing is strict so the existing 50,000-item optimization cannot widen
one changed type into an all-type reindex. An index created before signature metadata
exists receives one conservative full type reindex the first time selective mode is
